...
searchBtn.addEventListener("click", (_) => {
  if (inputSearch.value) getWeather(inputSearch.value);
});
inputSearch.addEventListener("input", async (_) => {
  clearTimeout(timeoutId);
  if (inputSearch.value && inputSearch.value.length > 2) {
    timeoutId = setTimeout(async () => {
      const result = await search(inputSearch.value);
      let e = "";
      result.forEach((city) => {
        e += `
...
function searchListEvent() {
  document.querySelectorAll(".search-list li").forEach((e) => {
    e.addEventListener("click", (_) => {
      getWeather(e.getAttribute("data-cityName"));
    });
  });
}
...
async function getWeather(city) {
  ...
  try {
    const req = await axios.get(`${baseUrl}weather?city=${city}`);
    addWeatherData(req.data);
  } catch (error) {
